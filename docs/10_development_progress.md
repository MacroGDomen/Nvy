...
### 2026-06-24 首页录屏反馈后的修正

用户提供 `D:/Users/Videos/Captures/Nvy 2026-06-24 13-47-12.mp4` 后，已实际读取视频并抽取连续关键帧确认首页问题。

已修正内容：

1. 首页女优和影片轮播中心点向右调整，使中心项更接近菜单栏收起后主内容区域的视觉中心。
2. 轮播方向从旧的逆时针 / 向左推进改为顺时针 / 项目整体向右移动。
3. 轮播窗口左右两侧额外保留过渡项，避免最左侧新项目突然出现、最右侧离场项目突然消失。
4. 轮播动效改为接近 1 秒的平滑位移、缩放和透明度过渡。
5. 修正影片轮播层级：左侧靠近中心的项目显示在右侧靠近中心项目上方，避免第 4 部遮住第 3 部。
6. 同步更新 `docs/08_page_wireframes.md`，将首页轮播规则改为顺时针、右侧渐隐离场、左侧渐显入场。
...
用户继续反馈首页轮播和详情返回行为后，已完成以下调整：

1. 女优轮播中心项使用与影片轮播中心项相同的横向坐标常量，确保两层轮播中间项对齐。
2. 女优和影片轮播间隔从 1 秒一次调整为 3 秒一次。
3. 从首页轮播点击进入女优或影片详情时，详情页左上角按钮显示并执行“返回首页”；从库页点击进入详情时仍返回对应库页。
...
## 25. 2026-06-24 首页女优轮播中心截图反馈修正

### 问题现象

用户提供首页截图后确认：影片轮播中间项位置正确，但女优轮播中间项明显偏右，没有与影片轮播的视觉中心对齐。

### 修正内容

| 文件 | 修改 |
|---|---|
| `src/pages/HomePage.tsx` | 将首页轮播中心坐标拆分为影片和女优两个独立常量。影片轮播继续使用已确认正确的 `56%` 中心；女优轮播改用独立的 `50%` 视觉中心，并同步左移左右弧线槽位，避免只移动中间项导致弧线形态变形。 |
...
需要用户在真实 Tauri 桌面窗口中查看首页，确认女优轮播中心项是否已经与影片轮播中心项视觉对齐。如果仍有少量偏差，下一轮只微调 `HOME_ACTRESS_CAROUSEL_CENTER_X`，不要改动影片轮播中心坐标。

## 26. 2026-06-24 设置页滚动与默认窗口比例修正

### 问题现象

用户提供设置页截图后确认两个问题：

1. 当窗口较小时，设置页无法通过鼠标滚轮看到下面的设置项。
2. 软件默认窗口大小下，16:9 舞台上下出现两条黑边。

### 修正内容

| 文件 | 修改 |
|---|---|
| `src/pages/SettingsPage.tsx` | 将设置页根容器从 `min-h-screen` 改为复用 `nvy-page-scroll`，让设置页在 16:9 舞台内部形成稳定纵向滚动区域，小窗口下可以继续向下滚动查看 API Key、导入导出和标签库等内容。 |
| `src-tauri/tauri.conf.json` | 将默认窗口尺寸从 `1280x800` 改为 `1280x720`，最小窗口从 `1024x680` 改为 `1024x576`，使默认 WebView 内容区与 16:9 舞台比例一致，避免默认启动时出现上下黑边。 |
...
需要在真实 Tauri 桌面窗口中复验：默认启动时是否不再出现上下黑边；缩小窗口后设置页是否可以通过滚轮滚到最下方。

## 27. 2026-06-24 设置页布局错位与大模型设置空白修正

### 问题现象

用户提供设置页截图后确认三个问题：

1. 数据导入导出区域在右侧窄栏中错位，导出路径、密码输入框和按钮互相重叠。
2. 大模型设置板块标题和第一行配置项之间存在一大块无意义空白。
3. 大模型设置中接口类型选择第二个接口后，触发按钮变得过长，并与旁边输入框重叠。

### 修正内容

| 文件 | 修改 |
|---|---|
| `src/pages/SettingsPage.tsx` | 大模型设置卡片和表单增加 `content-start`，避免 CSS Grid 在高卡片内拉伸行距造成大块空白。 |
| `src/pages/SettingsPage.tsx` | 数据导入导出内部布局从窄栏里的 `xl:grid-cols-2` 改为单列，并为导入导出卡片增加 `min-w-0`，避免右侧窄栏内横向溢出和控件重叠。 |
| `src/components/ui/Dropdown.tsx` | 下拉触发按钮增加 `min-w-0`、`overflow-hidden` 和文本 `truncate`，长选项名称只在按钮内截断，不再撑开网格列。 |
| `src/components/ui/Input.tsx` | 输入框和外层 label 增加 `w-full min-w-0`，防止设置页窄列中的输入框撑破父容器。 |
...
需要在真实 Tauri 桌面窗口中复验：设置页顶部大模型设置不再有大块空白；接口类型选择 `OpenAI 兼容 Chat Completions` 后不再与 Base URL 重叠；数据导入导出区域在右侧栏内按单列正常显示。

## 28. 2026-06-24 设置页本地存储位置调整

### 问题现象

用户提供设置页滚动截图后确认：大模型设置下方仍有大段空白，希望将设置页里的“本地存储”部分放到“大模型设置”下面。

### 修正内容

| 文件 | 修改 |
|---|---|
| `src/pages/SettingsPage.tsx` | 设置页顶部双列布局调整为：左列纵向显示“大模型设置”和“本地存储”；右列只显示“数据导入导出”。这样大模型设置结束后，本地存储会立即跟在下方，减少左列空白。 |
